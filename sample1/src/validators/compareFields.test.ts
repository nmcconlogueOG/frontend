...
describe('compareFields', () => {
  describe('LT — field1 must be less than field2', () => {
    it('passes when field1 < field2 (number)', () => {
      ...
      compareFields({ field1: 'a', field2: 'b', type: 'number', op: 'LT' })({ a: '1', b: '2' }, errors)
      expect(getErrors(errors, 'b')).toHaveLength(0)
    })

    it('fails when field1 > field2 (number)', () => {
      const errors = makeErrorsMock()
      compareFields({ field1: 'a', field2: 'b', type: 'number', op: 'LT' })({ a: '5', b: '2' }, errors)
      expect(getErrors(errors, 'b')).toHaveLength(1)
    })

    it('fails when field1 === field2 (equal is not strictly less than)', () => {
      const errors = makeErrorsMock()
      compareFields({ field1: 'a', field2: 'b', type: 'number', op: 'LT' })({ a: '5', b: '5' }, errors)
      expect(getErrors(errors, 'b')).toHaveLength(1)
    })

    it('passes for date comparison when end is after start', () => {
      const errors = makeErrorsMock()
      compareFields({ field1: 'start', field2: 'end', type: 'date', op: 'LT' })
    ...
    it('passes for datetime comparison', () => {
      const errors = makeErrorsMock()
      compareFields({ field1: 'from', field2: 'to', type: 'datetime', op: 'LT' })
    ...
    it('passes for string lexicographic comparison', () => {
      const errors = makeErrorsMock()
      compareFields({ field1: 'a', field2: 'b', type: 'string', op: 'LT' })
  ...
  describe('GT — field1 must be greater than field2', () => {
    it('passes when field1 > field2 (number)', () => {
      const errors = makeErrorsMock()
      compareFields({ field1: 'a', field2: 'b', type: 'number', op: 'GT' })({ a: '10', b: '5' }, errors)
      expect(getErrors(errors, 'b')).toHaveLength(0)
    })

    it('fails when field1 < field2 (number)', () => {
      const errors = makeErrorsMock()
      compareFields({ field1: 'a', field2: 'b', type: 'number', op: 'GT' })({ a: '3', b: '8' }, errors)
      expect(getErrors(errors, 'b')).toHaveLength(1)
    })

    it('fails when field1 === field2', () => {
      const errors = makeErrorsMock()
      compareFields({ field1: 'a', field2: 'b', type: 'number', op: 'GT' })({ a: '5', b: '5' }, errors)
      expect(getErrors(errors, 'b')).toHaveLength(1)
    })
  })

  describe('error attachment', () => {
    it('attaches the error to field2', () => {
      const errors = makeErrorsMock()
      compareFields({ field1: 'a', field2: 'b', type: 'number', op: 'LT' })({ a: '5', b: '2' }, errors)
    ...
    it('uses a custom message', () => {
      const errors = makeErrorsMock()
      compareFields({ field1: 'a', field2: 'b', type: 'number', op: 'LT', message: 'Custom!' })
        ({ a: '5', b: '2' }, errors)
    ...
    it('resolves nested dot-path fields', () => {
      const errors = makeErrorsMock()
      compareFields({ field1: 'group.a', field2: 'group.b', type: 'number', op: 'LT' })
  ...
  describe('skips validation', () => {
    it('when field1 is missing', () => {
      const errors = makeErrorsMock()
      compareFields({ field1: 'a', field2: 'b', type: 'number', op: 'LT' })({ b: '2' }, errors)
      expect(getErrors(errors, 'b')).toHaveLength(0)
    })

    it('when field2 is missing', () => {
      const errors = makeErrorsMock()
      compareFields({ field1: 'a', field2: 'b', type: 'number', op: 'LT' })({ a: '5' }, errors)
      expect(getErrors(errors, 'b')).toHaveLength(0)
    })

    it('when formData is undefined', () => {
      const errors = makeErrorsMock()
      compareFields({ field1: 'a', field2: 'b', type: 'number', op: 'LT' })(undefined, errors)
      expect(getErrors(errors, 'b')).toHaveLength(0)
    })
  })
})
